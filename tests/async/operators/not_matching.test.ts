...
describe('AsyncRelation.not_matching', () => {
  ...
  it('returns tuples from left that do not match right on common attrs', async () => {
    const got = await asyncOrders().not_matching(asyncCustomers()).toRelation();
    const expected = syncOrders().not_matching(syncCustomers());
    expect(isEqual(got, expected)).to.be.true;
  });

  it('supports explicit keys as [common_attr]', async () => {
    const got = await asyncOrders().not_matching(asyncCustomers(), ['customer_id']).toRelation();
    const expected = syncOrders().not_matching(syncCustomers(), ['customer_id']);
    expect(isEqual(got, expected)).to.be.true;
  });
  ...
  it('supports explicit keys as { left: right }', async () => {
    ...
    const got = await asyncSuppliers.not_matching(asyncCities, { city: 'location' }).toRelation();
    const expected = syncSuppliers.not_matching(syncCities, { city: 'location' });
    expect(isEqual(got, expected)).to.be.true;
  });
});
