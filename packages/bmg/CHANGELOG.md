...
- **`left_join` defaults (`COALESCE`)**: `.left_join(other, keys, defaults)` now accepts a `defaults` map. Unmatched right-side attributes (or matched attributes with a `null` value) take the default instead of `null`. In bmg-sql this compiles to `COALESCE(t.col, default) AS col` in the select list. Usage: `suppliers.left_join(supplies, ['sid'], { pid: 'P9', qty: 0 })`. New type export: `LeftJoinDefaults<R>`.
- **`rxmatch` operator**: substring match across multiple string attributes. Usage: `r.rxmatch(['city', 'name'], 'S')` → restricts to tuples where either attribute contains `'S'`. Options: `{ caseSensitive: false }` for case-insensitive matching (wraps both sides in `UPPER()` in SQL, `toUpperCase` in memory). In `@enspirit/bmg-sql` this compiles to a disjunction of `LIKE '%pattern%' ESCAPE '\'` predicates. New type export: `RxmatchOptions`.
- **`match` predicate** (in `@enspirit/predicate`): new `MatchPredicate` kind for LIKE-style substring matches with an optional `caseSensitive` flag. Builder: `Pred.match(attr, pattern, opts?)`. Evaluator returns false for non-string values. `toSql()` emits `LIKE '%pattern%' ESCAPE '\'` (or the `UPPER()`-wrapped form for case-insensitive).
- **`union` `all` option**: `.union(other, { all: true })` (UNION ALL) skips the relation-invariant deduplication step. In `@enspirit/bmg-sql` this compiles to `UNION ALL`. In memory the branches are simply concatenated. Use when the branches are known to be disjoint or duplicates are acceptable. New type export: `UnionOptions`.
- **`transform` tokens**: `Transformation` now accepts declarative tokens (`'string'`, `'integer'`, `'date'`) alongside JS functions. Usage: `r.transform({ qty: 'string' })`, `r.transform({ qty: ['string', 'integer'] })`, or `r.transform('string')` (all attrs). Token-only transforms push down to SQL — `'string'` → `CAST(col AS varchar(255))`, `'integer'` → `CAST(col AS integer)`, `'date'` → `date(col)` (SQLite function). Transforms containing any JS function fall back to in-memory. New type exports: `TransformToken`, `TransformStep`.
- **`@enspirit/bmg-sql`: restrict push-down across set ops**: `restrict` / `where` / `exclude` applied to a `union` / `intersect` / `except` now pushes the predicate into both branches rather than wrapping the set op in a derived table. `σ_p(A ∪ B) = σ_p(A) ∪ σ_p(B)` — semantically identical, one fewer subquery hop.
